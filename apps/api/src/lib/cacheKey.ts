...
export const buildProfileCachePatternForProfile = (input: { environment: Environment; profileId: string }): string => {
  return `${PROFILE_CACHE_PREFIX}:${encodeSegment(input.environment)}:${encodeSegment(input.profileId)}:*`;
};
