  ...
  return (
    <section className="space-y-4">
      <PageHeader
        density="compact"
        title="Asset Health"
        description="Operational health for governed offers, reusable assets, variants, and bundles."
        meta="Health is a readiness and operational risk signal. It is not attribution, ranking, or business performance truth."
      />

      {error ? <InlineError title="Asset health unavailable" description={error} /> : null}

      <FilterPanel density="compact" className="!space-y-0 flex flex-wrap items-center gap-3">
        <FieldLabel>
          Asset type
          <select className={`${inputClassName} ml-2 inline-block w-auto`} value={type} onChange={(event) => setType(event.target.value as typeof type)}>
            <option value="">All</option>
            <option value="offer">Offers</option>
            <option value="content">Reusable assets</option>
      ...
      <section className="grid gap-3">
        {(response?.items ?? []).map((item) => (
          <article key={`${item.type}:${item.key}`} className="panel grid gap-3 p-3 md:grid-cols-[minmax(240px,320px)_minmax(0,1fr)_auto] md:gap-5">
            <div className="min-w-0">
              <p className="text-xs uppercase tracking-wide text-stone-500">{item.type}</p>
              <h2 className="break-words text-base font-semibold leading-snug">{item.key}</h2>
            ...
            <div className="md:justify-self-end">
              <SignalChip tone={item.health === "healthy" ? "success" : item.health === "warning" ? "warning" : "danger"}>{item.health}</SignalChip>
            </div>
          </article>
        ))}
        {response && response.items.length === 0 ? <EmptyState title="No assets found" className="p-4" /> : null}
      </section>
    </section>
  );
}
